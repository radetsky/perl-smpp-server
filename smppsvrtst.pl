...
my $seq = $cli->bind_transceiver() or die;
my $pdu = $cli->read_pdu()         or die;
...
for (my $cx = 0; $cx < 1000; $cx++ ) {
	$seq = $cli->submit_sm('destination_addr' => '380504139380', short_message => 'Test message from smppsvrtst.pl') or die;
	$pdu = $cli->read_pdu() or die;
	if ($pdu->{status} != 0x00 ) { ## STATUS
