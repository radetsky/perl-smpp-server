...
if ( $pdu->{status} == 0x0000000E ) {    ## INVALID PASSWORD
    print
"ok 2: correct answer for empty bind_transceiever PDU. Invalid Password. \n";
}
else {
...
$seq = $cli->bind_transceiver( system_id => 'SMSGW', password => 'secret' )
  or die;
$pdu = $cli->read_pdu() or die;
...
if ( $pdu->{status} == 0x00 ) {    ## STATUS
    ...
    die
      "fail 4: single message was not acccepted. PDU->status must be 0x00. \n";
}
...
$seq =
  $cli3->bind_transceiver( system_id => 'test1000', password => 'secret1000' )
  or die;
...
for ( my $cx = 0 ; $cx < 1000 ; $cx++ ) {
    ...
    if ( $pdu->{status} != 0x00 ) {    ## STATUS
        die
"fail 5: multiple message was not acccepted. PDU->status must be 0x00. \n";
    }
}
...
"insert into messages ( msg_type, esme_id, src_addr, dst_addr, body, coding, udh, mwi, mclass, message_id, validity, deferred, registered_delivery, service_type, extra, received ) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ) "
);
$sth->execute(
    'MO',                        1,
    '0504139380',                'smppsvrtst.pl',
    conv_str_hex('bla-bla-bla'), 2,
    undef,                       undef,
    0,                           '1010101010101010101',
    1440,                        undef,
    undef,                       undef,
    undef,                       '2010-08-30 23:59:00'
);
$pdu = $cli->read_pdu();
...
$seq = $cli2->bind_transceiver( system_id => 'SMSGW', password => 'secret' )
  or die;
...
{    ## STATUS
    print
"ok 11: can't connect more than 1 time for SMSGW with NULL (default 1) max_connections.\n";
}
else {
    warn Dumper($pdu);
...
for my $test (
    qw(abcdefgh abcdefg abcedf abcde abcd abc ab a abcdefghi abcdefghij abcdefghabcdefgh)
  )
{
